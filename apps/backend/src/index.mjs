import { openStorage, applicationDataDirectory } from './storage.mjs';
...
export async function createBackend({
    settingsPath,
    dataDirectory = settingsPath
        ? dirname(settingsPath)
        : applicationDataDirectory(),
    legacyDirectory = settingsPath ? dirname(settingsPath) : dataDirectory,
    fetchImpl = fetch,
    codexProcessFactory,
    credentialCodec,
    openExternal,
    ...
    const storage = await openStorage({
        databasePath: join(dataDirectory, 'junior-mode.sqlite'),
        legacyDirectory,
    ...
    try {
        ...
        try {
            const saved = storage.getSetting('connection');
            if (saved) {
                state = {
                    ...state,
            ...
            connectPlatform: (value) =>
                serialize(async () => {
                    ...
                    if (next.status === 'connected') {
                        ...
                        storage.setSetting('connection', { platformUrl: url });
                    }
            ...
            checkPlatform: () =>
                serialize(async () => {
                    if (!state.platformUrl) {
                        throw new Error(
                            'Connect to a learning platform first.',
                        );
                    }
