    ...
    return (
        <dl className="grid gap-2 text-sm">
            ...
            <div>
                ...
                <dd>
                    {label(evidence.assistance)} · {evidence.hints_used} hints ·{' '}
        ...
        >
            <div className="grid gap-5">
                ...
                {progress.competencies.map((competency) => (
                    ...
                    >
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">
                            Suggested support:{' '}
                            {label(competency.suggested_support)}
                        </p>
                        {competency.assistance_trend.length > 0 && (
                            <p className="mt-2 text-sm">
                                ...
                                {competency.assistance_trend
                                    .map(
                                        (event) =>
                                            `${label(event.assistance)} (${event.hints_used} hints)`,
                                    )
                                    .join(' → ')}
                            </p>
                        )}
                        ...
                        {competency.correction_history.map((correction) => (
                            ...
                            >
                                ...
                                <p className="my-2">{correction.reason}</p>
                                <EvidenceSummary
                ...
                {receipts.map((receipt) => (
                    <article key={receipt.id} className="rounded-lg border p-4">
                        <h3 className="font-medium">
                            Learning receipt: {receipt.title}
                        </h3>
                        <p className="mt-2 text-sm">
                        ...
                        <p className="mt-2 text-sm">
                            Next challenge: {receipt.completion.next_challenge}
                        </p>
                        {receipt.completion.unresolved_questions.map(
                            (question) => (
                                <p className="mt-2 text-sm" key={question}>
