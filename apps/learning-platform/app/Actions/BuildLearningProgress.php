...
class BuildLearningProgress
{
    ...
    public function handle(User $learner, ?int $competencyId = null): array
    {
        ...
        return [
            ...
            'competencies' => $competencies->map(function ($competency) use ($current, $events, $escapedSessions): array {
                ...
                $independent = $qualified->filter(fn (LearningEvidence $event) => ! $escapedSessions->contains($event->coaching_session_id) && $event->evidence['assistance'] === 'review_only' && $event->evidence['hints_used'] === 0);
                ...
                return [
                    ...
                    'supporting_evidence' => $evidence->map(fn (LearningEvidence $event) => ['id' => $event->id, 'session_id' => $event->coaching_session_id, 'qualifies' => $qualified->contains('id', $event->id), 'recorded_at' => $event->created_at->toIso8601String(), 'recorded_by_id' => $event->recorded_by_id, 'client_connection_id' => $event->client_connection_id, 'solution_escape_used' => $escapedSessions->contains($event->coaching_session_id), 'evidence' => $event->evidence])->all(),
